...
async function runJourney(browser, config) {
  ...
  try {
    ...
    assert.ok(metrics.brandIntro >= 3_000, `Brand Intro sequence completed in only ${metrics.brandIntro}ms`);
    assert.ok(metrics.brandIntro < 4_300, `Brand Intro remained for ${metrics.brandIntro}ms after display`);
