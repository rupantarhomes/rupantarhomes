...
test("public inquiry delivery stays server mediated and rate limited", async () => {
  const [repository, cloudflare, edge, restriction] = await Promise.all([
...
test("Admin Work media keeps atomic persistence and reference-safe cleanup", async () => {
  const [repository, saveMigration, cleanupMigration, cloudinary] = await Promise.all([
    read("../app/rupantar/repository.ts"),
