...
-- The canonical Work RPC must not depend on the retired pre-blog overload.
-- Keep the exact Admin authorization, category validation, image limits,
-- cleanup-claim locking, persistence behavior, and blog URL normalization in
-- one forward-compatible function.
...
begin
  ...
  -- Lock every requested Cloudinary ID before persistence. If cleanup already
  -- claimed one, reject the stale save instead of re-referencing an asset that
